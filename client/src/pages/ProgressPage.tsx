...
import { makeSkillEvidence, skillLabels, suggestFields, type StrengthSkill } from "@/data/strengthSignals";
...
const gameMeta: { id: GameId; label: string; note: string; total: number; href: string }[] = [
  ...
  { id: "pattern-builder", label: "Pattern Builder", note: "Find the next color and shape", total: 5, href: "/discovery-lab?game=pattern-builder" },
  { id: "memory-adventure", label: "Memory Adventure", note: "Remember where the star was", total: 5, href: "/discovery-lab?game=memory-adventure" },
  { id: "mini-scientist", label: "Mini Scientist", note: "Predict what the change will do", total: 5, href: "/discovery-lab?game=mini-scientist" },
  { id: "bridge-builder", label: "Bridge Builder", note: "Make a path that can hold", total: 5, href: "/discovery-lab?game=bridge-builder" },
  { id: "robot-programmer", label: "Robot Programmer", note: "Guide a bot with arrows", total: 5, href: "/discovery-lab?game=robot-programmer" },
  { id: "traffic-controller", label: "Traffic Controller", note: "Keep the moving colors safe", total: 5, href: "/discovery-lab?game=traffic-controller" },
  { id: "mission-commander", label: "Mission Commander", note: "Choose the next safe step", total: 5, href: "/discovery-lab?game=mission-commander" },
  { id: "creative-studio", label: "Creative Studio", note: "Build a balanced visual scene", total: 5, href: "/discovery-lab?game=creative-studio" },
  { id: "performance-arena", label: "Performance Arena", note: "Follow the silent beat", total: 5, href: "/discovery-lab?game=performance-arena" },
];

const visualGameIds: GameId[] = ["pattern-builder", "memory-adventure", "mini-scientist", "bridge-builder", "robot-programmer", "traffic-controller", "mission-commander", "creative-studio", "performance-arena", "robotics"];
const visualSkillMap: Partial<Record<GameId, StrengthSkill[]>> = {
  "pattern-builder": ["pattern-recognition", "sequencing"],
  "memory-adventure": ["visual-memory", "attention"],
  "mini-scientist": ["observation", "cause-effect"],
  robotics: ["planning", "systems-thinking", "problem-solving"],
  "bridge-builder": ["spatial-reasoning", "planning", "optimization"],
  "robot-programmer": ["sequencing", "computational-thinking", "problem-solving"],
  "traffic-controller": ["attention", "decision-making", "timing"],
  "mission-commander": ["planning", "adaptability", "decision-making"],
  "creative-studio": ["design-thinking", "visual-creativity", "planning"],
  "performance-arena": ["timing", "rhythm", "attention"],
};
...
const badgeMeta = [
  ...
  { id: "visual", label: "Visual explorer", note: "Try a color, memory or science game", icon: Sparkles },
];
...
export default function ProgressPage() {
  ...
  const visualEvidence = Array.from(new Set(visualGameIds.filter((id) => progress[id].completions > 0).flatMap((id) => visualSkillMap[id] ?? []))).map((skill) => { const sourceGames = visualGameIds.filter((id) => progress[id].completions > 0 && visualSkillMap[id]?.includes(skill)); const evidenceCount = sourceGames.reduce((sum, id) => sum + Math.min(progress[id].completions, 3), 0); return makeSkillEvidence(skill, evidenceCount, sourceGames); }).sort((a, b) => b.evidenceCount - a.evidenceCount);
  const fieldSuggestions = suggestFields(visualEvidence.map((evidence) => evidence.skill));
  const reset = () => { resetLearningProgress(); setProgress(getLearningProgress()); trackLearningEvent("learning_progress_reset"); };
  return <div className="progress-page"><header className="progress-header"><a className="game-brand" href="/"><span className="progress-logo">✦</span><span><strong>AI</strong> for <em>Students</em></span></a><a className="game-return" href="/"><ArrowLeft size={15} /> Back to the desk</a><AuthControls /></header><main className="progress-main"><div className="progress-top"><div><span className="game-kicker">YOUR LEARNING DESK</span><h1>Keep track of<br /><em>what is sticking.</em></h1><p>{isAuthenticated ? "Your progress is saved to your account and available when you return." : "Your progress stays on this device. Play as a guest, or sign in to keep it across devices."}</p>{!isAuthenticated && <LocalAuthDialog label="Sign in to save this progress" className="text-link account-save-link" />}</div><div className="progress-stats"><div><strong>{completed}/{gameMeta.length}</strong><small>games complete</small></div><div><strong>{totalAttempts}</strong><small>sessions started</small></div></div></div><section className="dashboard-section"><div className="dashboard-section-head"><span className="game-kicker">THE SHELF / 01</span><h2>Learning<br /><em>in motion.</em></h2></div><div className="dashboard-games">{gameMeta.map((game) => { const item = progress[game.id]; return <a className="dashboard-game" href={game.href} key={game.id}><div className="dashboard-game-top"><span>{game.label}</span><BarChart3 size={16} /></div><strong>{item.bestScore}<small> / {game.total}</small></strong><p>{item.completions ? `${item.completions} completed · ${game.note}` : `Not started · ${game.note}`}</p><div className="dashboard-bar"><span style={{ width: `${Math.min(100, (item.bestScore / game.total) * 100)}%` }} /></div><span className="dashboard-action">{item.completions ? "Play again" : "Open game"} <ArrowRight size={14} /></span></a>; })}</div></section><section className="dashboard-section visual-signal-card"><div className="dashboard-section-head"><span className="game-kicker">DISCOVERY LAB / PRACTICE SIGNALS</span><h2>What your play<br /><em>is exploring.</em></h2></div><p className="visual-signal-disclaimer">{visualEvidence.length ? "These are signals from the games you completed—not a test result or fixed label. Play varied worlds before drawing conclusions." : "Complete a visual game to start a private, local practice snapshot."}</p>{visualEvidence.length > 0 && <div className="visual-signal-list">{visualEvidence.map((evidence) => <span key={evidence.skill}>{skillLabels[evidence.skill]} <small>{evidence.signal} · {evidence.sourceGames.length} game{evidence.sourceGames.length === 1 ? "" : "s"} · based on current gameplay</small></span>)}</div>}{fieldSuggestions.length > 0 && <div className="field-suggestion-list"><strong>Fields worth exploring</strong><small>Only conversation starters from varied play—not predictions.</small>{fieldSuggestions.slice(0, 3).map((suggestion) => <span key={suggestion.field}>{suggestion.field}<small>{suggestion.matchedSkills.length} matching practice signals</small></span>)}</div>}<a className="dashboard-action" href="/discovery-lab">Open Discovery Lab <ArrowRight size={14} /></a></section>{isAuthenticated && <section className="dashboard-section explorer-progress-card"><div className="dashboard-section-head"><span className="game-kicker">EXPLORER LAB / PRIVATE EVIDENCE</span><h2>Practice that<br /><em>travels with you.</em></h2></div><div className="explorer-progress-card-body"><div><strong>{explorerAttemptsQuery.data?.length ?? 0}</strong><span>saved Explorer missions</span><small>Only parent-consented evidence appears here.</small></div><a className="dashboard-action" href="/explorer/report">Open private report <ArrowRight size={14} /></a></div></section>}<section className="dashboard-section dashboard-badges">
<div className="dashboard-section-head"><span className="game-kicker">THE PINBOARD / 02</span><h2>Small marks<br /><em>of progress.</em></h2></div><div className="badge-grid">{badgeMeta.map((badge) => { const earned = badge.id === "first" ? completed > 0 : badge.id === "detective" ? progress["prompt-detective"].bestScore >= 4 : badge.id === "evidence" ? progress["fact-check-quest"].completions > 0 : badge.id === "safety" ? progress["ai-safety-lab"].completions > 0 : badge.id === "robot" ? progress.robotics.completions > 0 : badge.id === "visual" ? visualGameIds.some((id) => progress[id].completions > 0) : completed === gameMeta.length; const Icon = badge.icon; return <div className={`badge-card ${earned ? "is-earned" : ""}`} key={badge.id}><span className="badge-icon">{earned ? <Icon size={19} /> : <LockKeyhole size={17} />}</span><strong>{badge.label}</strong><small>{badge.note}</small>{earned && <span className="badge-earned"><Check size={12} /> Earned</span>}</div>; })}</div></section><div className="progress-footer-actions"><a className="text-link" href="/play"><ArrowLeft size={15} /> Back to learning games</a><button className="reset-progress" onClick={reset}><RotateCcw size={14} /> Reset local progress</button></div></main><footer className="game-page-footer"><span>AI for Students</span><span>Learn the tool. Keep the thinking.</span></footer></div>;
}
