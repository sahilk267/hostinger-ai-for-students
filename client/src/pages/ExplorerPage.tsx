...
import { EXPLORER_LOCALES, explorerCopy, getExplorerLocale, quickPlayOptions, speakExplorerText, visualModeCards, visualModeCopy, visualModeForKind, type ExplorerLocale, type ExplorerVisualMode } from "@/data/explorerI18n";
...
export default function ExplorerPage() {
  ...
  const completeMission = () => {
    if (!mission) return;
    const missing = (["choice", "reason"] as ExplorerEvidenceField[]).filter((field) => !evidence[field]?.trim());
    if (!visualDone) {
          ...
          <section className="explorer-mission-view" aria-labelledby="explorer-mission-heading">
            ...
            <div className="explorer-mission-layout">
              ...
              <div className="explorer-evidence-card">
                <div className="explorer-evidence-head"><div><span className="mission-section-label">STEP 02 / ANSWER</span><h3 id="explorer-answer-heading">{selectedRecord ? "Your evidence is saved." : "Choose and explain your move"}</h3></div><div className="explorer-evidence-head-actions"><button type="button" className="explorer-listen" onClick={() => readAloud(`${missionCopy?.scenario ?? mission.scenario}. ${missionCopy?.task ?? mission.task}`)}><Ear size={16} /> {isSpeaking ? copy.listening : copy.listen}</button>{selectedRecord && <Trophy size={22} />}</div></div>
                <div className="explorer-answer-controls" aria-labelledby="explorer-answer-heading">
                  <div className="explorer-visual-choice"><span>{visualModeCopy[visualMode][locale].instruction}</span><div className="explorer-visual-selects">{Array.from({ length: visualMinimum }, (_, slot) => <label key={slot}><span>{slot === 0 ? copy.firstMove : copy.nextMove}</span><select aria-label={slot === 0 ? copy.firstMove : copy.nextMove} value={visualSelections[slot] ?? ""} onChange={(event) => event.target.value && chooseVisualCard(event.target.value, slot)} disabled={Boolean(selectedRecord)}><option value="">{copy.chooseCard}</option>{visualCardLabels.map((card) => <option key={card} value={card}>{card}</option>)}</select></label>)}</div><p className="explorer-visual-status">{visualDone ? <><Check size={15} /> {visualModeCopy[visualMode][locale].done}</> : `${visualSelections.length} / ${visualMinimum}`}</p></div>
